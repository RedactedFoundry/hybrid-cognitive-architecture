...
// Dark-only. Generous spacing. Slightly more purple accents.
...
const PURPLE = "#6d5bd0";          // extra purple accent
...
export default function HACPreview() {
  ...
  // Full-screen drill-downs (independent for each section)
  const [showTreasury, setShowTreasury] = useState(false);
  const [showAgents, setShowAgents] = useState(false);
  const [showPheromind, setShowPheromind] = useState(false);
  const [agentDetail, setAgentDetail] = useState(null); // selected agent for detail view

  // Conversation list (ChatGPT-style only — removed projects metaphor)
  ...
  // Chat — single bubble with status progression (Generating → Verifying → Complete)
  ...
  const streamRef = useRef({ iid: null, aId: null, timeouts: [] });
  ...
  const send = () => {
    ...
    setComposer("");
    autosize();
    // assistant bubble (single, progressive)
    const aId = uid();
    ...
    const t1 = setTimeout(() => {
      ...
      setMessages((xs) => xs.map((m) => (m.id === aId ? { ...m, text: streamText + "Preparing final answer…", status: "verifying" } : m)));
    }, 900);

    // complete + verified
    const t2 = setTimeout(() => {
      setMessages((xs) => xs.map((m) => (m.id === aId ? { ...m, text: streamText + "Final: Based on volatility and liquidity, consider trimming BTC by 5% and allocating to ETH and cash equivalents.", status: "complete", verified: true } : m)));
      setStreaming(false);
      setStreamIdx(0);
      streamRef.current.iid = null;
    }, 1500);

    streamRef.current.timeouts.push(t1, t2);
  };

  const stop = () => {
    if (streamRef.current.iid) clearInterval(streamRef.current.iid);
    streamRef.current.timeouts.forEach((t) => clearTimeout(t));
    setStreaming(false); setStreamIdx(0);
    // mark current assistant bubble as complete (partial text)
    const aId = streamRef.current.aId;
    if (aId) setMessages((xs) => xs.map((m) => (m.id === aId ? { ...m, status: "complete" } : m)));
    streamRef.current.iid = null; streamRef.current.timeouts = []; streamRef.current.aId = null;
  };
  ...
  // Agents sample (separate from Pheromind)
  const agents = [
  ...
  // Mode color accents (used subtly)
  ...
  return (
    <div className="min-h-screen" style={{ background: BG, color: "#fff" }}>
      ...
      <div className="grid grid-cols-12 gap-8 mx-8 my-8">
          ...
          <div className="rounded-3xl p-8 border backdrop-blur-xl" style={{ background: GLASS.bg, borderColor: GLASS.border }}>
            ...
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <div className="text-sm" style={{ color: TXT_MUTED }}>Threaded messages • Mode-aware approvals • Streaming with verification</div>
              </div>
              <div className="flex items-center gap-3">
                <SlashQuickActions onPick={(cmd) => setComposer((t) => (t ? t + ` ${cmd} ` : cmd + " "))} />
                {streaming ? (
                  <button onClick={stop} className="px-4 py-3 rounded-xl text-base inline-flex items-center gap-2" style={{ background: GLASS.bg, border: `1px solid ${GLASS.border}` }}>
            ...
            {/* Composer — auto-size textarea (1→6 lines) */}
            <div className="mt-8 rounded-2xl p-6 border" style={{ background: GLASS.subtle, borderColor: GLASS.border }}>
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button onClick={() => {/* voice push-to-talk stub */}} className="p-3 rounded-xl border" style={{ background: GLASS.bg, borderColor: GLASS.border }}>
              ...
              <div className="mt-4 flex items-center justify-between">
                ...
                <button onClick={() => { setStreaming(true); setTimeout(send, 10); }} className="px-6 py-3 rounded-2xl text-lg font-medium border inline-flex items-center gap-2" style={{ background: ACCENT, borderColor: GLASS.border, boxShadow: `0 8px 30px ${PURPLE}33` }}>
        ...
        {/* Right panel — split into independent sections */}
      ...
      {/* Treasury — full-screen detail */}
      {showTreasury && (
        <Fullscreen onClose={() => setShowTreasury(false)} title="Treasury — Detail">
          <div className="grid grid-cols-3 gap-8">
            <KPI label="Available" value={`$${treasury.balance.toLocaleString()}`} />
            <KPI label="Allocated" value={`$${treasury.allocated.toLocaleString()}`} />
            <KPI label="Net P&L (wk)" value={`+$${treasury.pnlWeek}`} trend="up" />
          </div>
          <div className="mt-8 text-lg" style={{ color: TXT_MUTED }}>Spending trends, forecasts, thresholds (placeholder)</div>
          <div className="mt-6 rounded-2xl p-6 border" style={{ background: GLASS.subtle, borderColor: GLASS.border }}>Charts would render here.</div>
        </Fullscreen>
      )}

      {/* Agents — full-screen list + optional agent detail */}
      {showAgents && (
        <Fullscreen onClose={() => { setShowAgents(false); setAgentDetail(null); }} title={agentDetail ? `Agent — ${agentDetail.name}` : "KIP Agents — Detail"}>
          {!agentDetail ? (
            <div className="grid grid-cols-3 gap-6">
              {agents.map((a) => (
                <AgentCard key={a.id} a={a} onDetails={() => setAgentDetail(a)} />
              ))}
            </div>
          ) : (
      ...
      {/* Pheromind — full-screen detail */}
      {showPheromind && (
        <Fullscreen onClose={() => setShowPheromind(false)} title="Pheromind — Concept Trails">
    ...
    <div className="flex items-center gap-3">
      {(["MANUAL", "SUPERVISED", "AUTONOMOUS"]).map((m) => (
        <button key={m} onClick={() => setMode(m)} className={`px-5 py-3 rounded-2xl text-base font-medium border ${mode === m ? "" : "opacity-70"}`} style={{
          background: mode === m ? "rgba(109,91,208,0.25)" : GLASS.bg,
...
function ConvRow({ title, unread, active, onClick, collapsed }) {
  return (
    <button onClick={onClick} className={`w-full rounded-2xl text-left ${active ? "" : "opacity-90"}`}
      style={{ padding: 18, background: active ? ACCENT : GLASS.subtle, border: `1px solid ${GLASS.border}`, boxShadow: active ? `0 8px 24px ${PURPLE}33` : "none" }}>
...
function VoiceBars() {
  return (
    <div className="mt-4 rounded-2xl p-4 border" style={{ background: GLASS.subtle, borderColor: GLASS.border }}>
...
// Mini Pheromind preview (right panel card)
...
// Miro-like pheromind canvas (fullscreen)
